<template><div><p>深入RabbitMQ
加文·罗伊</p>
<p>◆ 1.1 RabbitMQ特性以及好处</p>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>[插图]图1.6 通过使用RabbitMQ，当将同一份数据同时发送到新的云服务和原本的数据库时，发布者应用程序不需要做任何改变</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>1.3.5 多主（Multi-Master）互联化数据与事件</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>针对代理服务器软件，AMQ模型在逻辑上定义了三种抽象组件用于指定消息的路由行为：■ 交换器（Exchange），消息代理服务器中用于把消息路由到队列的组件。■ 队列（Queue），用来存储消息的数据结构，位于硬盘或内存中。■ 绑定（Binding），一套规则，用于告诉交换器消息应该被存储到哪个队列。</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>[插图]图2.2 AMQP Connection类以及Start方法构成了Connection.Start RPC请求</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>■ 协议头帧用于连接到RabbitMQ，仅使用一次。■ 方法帧携带发送给RabbitMQ或从RabbitMQ接收到的RPC请求或响应。■ 内容头帧包含一条消息的大小和属性。</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>[插图]图2.11 当发送消息到RabbitMQ时，至少需要发送三个帧：Basic.Publish方法帧、内容头帧和消息体帧</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>■ 使用content-type属性让消费者知道如何解释消息体。■ 使用content-encoding属性来指示消息体使用某种特殊的方式进行压缩或编码。■ 填充message-id和correlation-id来唯一标识消息和消息响应，用于在工作流程中实现消息跟踪。■ 利用timestamp属性减少消息大小，并创建一个规范定义来描述消息创建时间。■ 使用expiration属性表明消息过期。</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
<p>■ 告诉RabbitMQ使用delivery-mode将消息写入磁盘或内存队列。■ 使用app-id和user-id来帮助追踪出现问题的消息发布者应用程序。■ 使用type属性来定义发布者和消费者之间的契约。■ 使用reply-to属性实现响应消息的路由。■ 使用headers映射表定义自由格式的属性和实现RabbitMQ路由。</p>
</blockquote>
</blockquote>
<p>◆ 3.2 使用content-type属性创建显式的消息契约</p>
<blockquote>
<blockquote>
...
<p>◆ 3.3 通过gzip和content-encoding属性压缩消息大小</p>
<blockquote>
<blockquote>
...
<p>◆ 3.4 使用message-id和correlation-id引用消息</p>
<blockquote>
<blockquote>
<p>3.4 使用message-id和correlation-id引用消息</p>
</blockquote>
...
<blockquote>
<blockquote>
<p>[插图]图3.5 message-id和correlation-id属性可用于跟踪单个消息及其响应消息在系统中的流动情况</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<p>◆ 3.7 使用delivery-mode平衡速度和安全性</p>
<blockquote>
<blockquote>
...
<p>◆ 3.8 使用app-id和user-id验证消息来源</p>
<blockquote>
<blockquote>
...
<p>◆ 3.13 不能使用的属性：cluster-id/reserved</p>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>由Basic.Properties提供的属性，包括它们的类型、代理服务器或应用程序是否可以使用它们，以及使用上的一些规范和建议[插图]</p>
</blockquote>
</blockquote>
<p>◆ 4.1 平衡投递速度与可靠投递</p>
<blockquote>
...
<blockquote>
<blockquote>
<p>在使用RabbitMQ创建应用程序体系结构时，应该牢记金发姑娘原则。下面的问题可以帮助找到刚刚好的解决方案，用于平衡高性能和消息传递的安全性。■ 消费发布时保证消息进入队列的重要性有多高？</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
<p>■ 如果消息无法路由，是否应将消息返回给发布者？■ 如果消息无法路由，是否应该将其发送到其他地方稍后进行重新路由？■ 如果RabbitMQ服务器崩溃，可以接受信息丢失吗？</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
<p>■ RabbitMQ在处理新消息时是否应该确认它已经为发布者执行了所有请求的路由和持久化任务？■ 消息发布者是否可以批量投递消息，然后从RabbitMQ收到一个确认用于表明所有请求的路由和持久化任务已经批量应用到所有的消息中？■ 如果你要批量发布消息，而这些消息需要确认路由和持久化，那么对每一条消息是否需要对目标队列实现真正意义上的原子提交？</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>用于查询RabbitMQ管理API以获取信道状态的API端点为http://host:port/api/channels/[CHANNEL_NAME]。查询时，管理API将以JSON序列化对象的格式返回结果。以下是使用API查询信道状态所返回的简略示例：[插图]</p>
</blockquote>
</blockquote>
<p>◆ 4.3 小结</p>
<blockquote>
...
<blockquote>
<blockquote>
<p>■ 发布者是否要求将消息持久化到磁盘？■ 应用程序的各个组件需要什么样的保障机制以确保发布的消息都会被接收？■ 如果应用程序被TCP背压阻塞，或者在将消息发布到RabbitMQ时连接被阻塞，我的环境中将会发生什么？■ 我的消息有多重要？我可以牺牲消息的可靠投递来实现更高的消息吞吐量吗？</p>
</blockquote>
</blockquote>
<p>◆ 5.1 对比Basic.Get和Basic.Consume</p>
<blockquote>
<blockquote>
<p>5.1 对比Basic.Get和Basic.Consume</p>
</blockquote>
...
<blockquote>
<blockquote>
<p>要增加Linux操作系统中接收套接字缓冲区的数量，我们应该增加net.core.rmem_default和net.core.rmem_max值（默认是128KB）。对于大多数环境而言，16MB （16777216）应该足够了。大多数Linux发行版都可以在/etc/sysctl.conf中更改此值，但也可以通过以下命令来手动设置值：[插图]</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>定义队列时，有多个设置可以确定队列的行为。队列至少可以做到以下几点：■ 自动删除自己。■ 只允许一个消费者进行消费。■ 自动过期的消息。■ 保持有限数量的消息。■ 将旧消息推出堆栈。</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>[插图]图6.10 topic交换器路由键由三部分组成</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
<p>如果我们想要以图片上传流程为基础，创建基于消息通信的架构来管理网站上所有图片相关的任务的话，那么下面列举的这些路由键可以用来描述其中一部分消息。● image.new.profile——用于表示包含新资料图片的消息。● image.new.gallery——用于表示包含新相册图片的消息。● image.delete.profile——用于表示含有删除资料图片的元数据的消息。● image.delete.gallery——用于表示含有删除相册图片的元数据的消息。● image.resize——用于表示要求调整图片大小的消息。</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>[插图]图6.16 公平随机哈希下的10万条消息的分布情况</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>[插图]图7.2 管理界面使用单个节点来突出表示集群状态</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
...
<blockquote>
<blockquote>
<p>[插图]图7.3 带有备用磁盘节点和两个内存节点的集群</p>
</blockquote>
</blockquote>
<blockquote>
<blockquote>
